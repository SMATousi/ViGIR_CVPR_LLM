...
####################################################################################################
####################################################################################################
################################## Visual Attributes Defining ######################################
####################################################################################################
####################################################################################################

visual_attributes = {
    "arts-and-crafts": [
        "Use of bright, varied colors (paints, markers, papers)",
        "Presence of crafting tools (scissors, glue, brushes)",
        "Handmade objects or DIY projects on display",
        "Textures like paper, fabric, or clay",
        "Patterns, shapes, or decorative motifs"
    ],
    "astronaut": [
        "Distinctive bulky spacesuit (often white)",
        "Large, reflective helmet visor",
        "Life-support backpack or hoses",
        "Mission patches or space agency insignia",
        "Zero-gravity or space-related backdrop"
    ],
    "block-tower": [
        "Stacked rectangular or square blocks",
        "Noticeable height or precarious balance",
        "Often multiple colors or sizes of blocks",
        "Playful or constructive setting (children play area)",
        "Clean, geometric shapes and lines"
    ],
    "dance": [
        "Dynamic or fluid body movement/poses",
        "Costumes or specialized dance attire",
        "Performance setting (stage, studio)",
        "Expressive gestures (arm/leg extensions)",
        "Possible musical context (instruments, dance floor)"
    ],
    "emergency-service": [
        "Vehicles like ambulances or fire trucks",
        "Uniformed personnel (EMTs, firefighters, police)",
        "Symbols of urgency (lights, sirens)",
        "Official badges, patches, or reflective gear",
        "Scenes of rescue or immediate response"
    ],
    "gourmet-tuna": [
        "High-quality, pink or red tuna cuts",
        "Artful plating or garnish (herbs, drizzle)",
        "Fine dining presentation (white plates, decorative sauces)",
        "Clean, delicate portions or slices",
        "A sense of freshness or premium quality"
    ],
    "hair-coloring": [
        "Visible hair dye or color swatches",
        "Salon tools (brushes, mixing bowls, foils)",
        "Foils or sections of hair separated",
        "Vibrant or noticeably altered hair tones",
        "A hairstylist or mirror in the background"
    ],
    "hand-pointing": [
        "Index finger clearly extended",
        "Emphasis on the gesture or direction",
        "Close-up of the hand or arm",
        "Minimal background distractions (focus on the hand)",
        "Suggestive of an instruction or command"
    ],
    "healthy-dish": [
        "Abundant fresh fruits or vegetables",
        "Lean proteins or whole grains",
        "Bright, natural colors on the plate",
        "Minimal sauces or light dressings",
        "Balanced portion sizes"
    ],
    "home-fragrance": [
        "Candles, diffusers, or incense sticks",
        "Attractive or minimalistic packaging (bottles, jars)",
        "Wisps of smoke or vapor (if in use)",
        "Cozy or tranquil home setting",
        "Soft lighting or warm ambience"
    ],
    "in-ear-headphones": [
        "Compact earpieces (wireless or wired buds)",
        "Small charging case (for wireless types)",
        "Snug fit in the ear canal",
        "Minimal, sleek design",
        "Either a single cable or no cable (if Bluetooth)"
    ],
    "pie-chart": [
        "Circular chart divided into slices",
        "Various colored segments",
        "Clear demarcation of percentages or segments",
        "A legend identifying each color",
        "Central pivot point where slices converge"
    ],
    "single-sneaker-on-white-background": [
        "Clearly defined silhouette of the shoe",
        "Visible details like laces, logos, or stitching",
        "Clean, bright white backdrop",
        "Focus on shoe color and texture",
        "Sole tread pattern often visible"
    ],
    "stop-sign": [
        "Octagonal shape",
        "Bright red color with white border",
        "Large, bold 'STOP' text",
        "Typically mounted on a metal pole",
        "High visibility for traffic safety"
    ]
}
...
run = wandb.init(
    # entity="jacketdembys",
    project=f"CVPR-2025-{args.dataset_name}-MC",
    name=f"run_test_{args.dataset_name}_"+args.model_name+"-"+args.llm_name+"-"+args.subset
)
...
################### Pulling LLM (DeepSeek) ###########################
ollama.pull(args.llm_name)
...
list_of_vis_attributes = visual_attributes[args.dataset_name]
print(f"The list of attributes: {list_of_vis_attributes}")

list_of_vis_prompts = []
for vis_att in list_of_vis_attributes:
    list_of_vis_prompts.append(f"Is there any evidence of {vis_att} in this image? Answer with Yes or No")
...
#disp_img(image_path)
    ...
    try:
        ...
        for prompt in list_of_vis_prompts:
        ...
        llm_response = ollama.generate(model=args.llm_name, prompt=llm_prompt, options=options)
        model_response = response['response']

        query_embedding = get_query_embedding(model_response, tokenizer, text_encoder)
    ...
    # print(f"{image_path} | {matched_label} | {model_response}")
        model_labels[key] = {
            ...
            "answers": questions_and_answers, # actual prompt used to generate embedding
